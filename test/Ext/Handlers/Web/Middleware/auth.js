...
describe('Web Middleware Auth:', () => {
  ...
  before((done) => {
    ...
    Oca.addBeforeAuthAction(passportAuth);
  ...
  after(() => {
    ...
    Oca.Ext.Handlers.Web._beforeAuthActionMiddlewares = [];
  ...
  it('Should fail to perform an action that requires auth without any login', (done) => {
    Oca.webfyAction(testutils.Actions.Shared.Sum, 'get', {auth: true, rest: false});

    app.get('/authSum', Oca.middleware('sum', (err, result, req, res, next) => {
